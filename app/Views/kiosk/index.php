...
<body class="bg-gray-50 h-screen flex flex-col overflow-hidden hero-pattern font-sans antialiased text-gray-800 select-none" x-data="kioskApp()">
    ...
    <!-- Header -->
    <header class="h-24 bg-white/80 backdrop-blur-xl border-b border-gray-200 sticky top-0 z-30 flex-none shadow-sm">
        <div class="h-full max-w-7xl mx-auto px-6 flex items-center justify-between">
            <div class="flex items-center gap-5">
                <div class="w-14 h-14 bg-gradient-to-br from-primary-600 to-primary-800 rounded-2xl flex items-center justify-center shadow-lg shadow-primary-500/30 text-white">
                ...
                <div>
                    ...
                    <p class="text-gray-500 font-medium tracking-wide">Anjungan Pendaftaran Mandiri</p>
                </div>
            </div>
    ...
    <!-- Main Content -->
    <main class="flex-1 relative overflow-hidden flex flex-col z-10 w-full max-w-7xl mx-auto px-6 py-8">
        <!-- Hero Title -->
        <div class="text-center mb-10 animate-float">
            <h2 class="text-4xl md:text-5xl font-extrabold text-gray-900 mb-4 tracking-tight">Selamat Datang</h2>
            <p class="text-xl text-gray-600 max-w-2xl mx-auto">Silakan sentuh salah satu layanan di bawah ini untuk mengambil nomor antrian.</p>
        </div>

        <!-- Grid Cards -->
        <!-- Grid Cards -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20 overflow-y-auto px-4 perspective no-scrollbar">
            <?php foreach($polis as $poli): ?>
            <button @click="ambilNomor(<?= $poli['id'] ?>)" class="card-3d group relative bg-white rounded-3xl p-6 shadow-xl border border-gray-100/50 hover:shadow-2xl hover:border-primary-400 overflow-hidden text-left flex items-center gap-6 h-32 md:h-40 transition-all duration-300 w-full outline-none focus:ring-4 ring-primary-200">
                ...
                <!-- Icon Box (Left) -->
                <div class="relative z-10 w-20 h-20 md:w-24 md:h-24 rounded-2xl shrink-0 bg-gradient-to-br <?= match($poli['prefix']) { 'A' => 'from-blue-50 to-blue-100 text-blue-600', 'B' => 'from-emerald-50 to-emerald-100 text-emerald-600', 'C' => 'from-purple-50 to-purple-100 text-purple-600', default => 'from-gray-50 to-gray-100 text-gray-600' } ?> flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform duration-300">
                    <span class="font-black text-4xl md:text-5xl font-mono"><?= $poli['prefix'] ?></span>
                </div>

                <!-- Content (Right) -->
                <div class="relative z-10 flex-1 flex flex-col justify-center h-full">
                    <div class="flex items-center justify-between mb-1">
                        <div class="px-2.5 py-1 rounded-lg bg-green-100 text-green-700 text-[10px] md:text-xs font-bold uppercase tracking-wider border border-green-200 inline-flex items-center gap-1.5">
                            <span class="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span>
                            Buka
                        </div>
                    </div>
                    
                    <h3 class="text-2xl md:text-3xl font-black text-gray-800 group-hover:text-primary-700 transition-colors leading-none truncate w-full"><?= esc($poli['nama']) ?></h3>
                    <p class="text-sm text-gray-400 font-medium group-hover:text-primary-500 mt-1 flex items-center gap-1">
    ...
    <!-- Footer -->
    <footer class="h-16 bg-white border-t border-gray-200 flex items-center justify-center text-gray-400 text-sm font-medium z-20 flex-none gap-8">
        <div class="flex items-center gap-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            Butuh bantuan? Silakan hubungi petugas resepsionis.
        </div>
    </footer>
